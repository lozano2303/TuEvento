...
/**
 * GoogleButton — botón de Google con tamaño y estructura idénticos al de Facebook.
 * Usa la API nativa window.google.accounts.id (GSI) para disparar el popup y
 * obtener el credential (id_token), sin depender del botón pre-renderizado de
 * @react-oauth/google que no es personalizable.
 * Debe estar dentro de GoogleOAuthProvider.
 */
function GoogleButton({ onSuccess, onError }) {
  const { googleScriptLoadedSuccessfully } = useGoogleOAuth();

  const handleClick = () => {
    if (!googleScriptLoadedSuccessfully || !window.google?.accounts?.id) {
      onError?.();
      return;
    }
    ...
    window.google.accounts.id.prompt((notification) => {
      if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
        // One Tap not available — fall back to the accounts selector popup
